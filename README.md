...
S3 (stores rounds.parquet + course\_meta.json)
...
&#x20;               - MCP server exposes execute\_sql + get\_course\_info tools
...
\- \*\*`execute\_sql`\*\* — Claude writes DuckDB SQL dynamically based on the question and the schema document in the system prompt. This means the agent can answer questions that were never anticipated at build time.

\- \*\*`get\_course\_info`\*\* — returns course metadata (hole pars, distances, layout) from a JSON file on S3. Used when the user asks about the course itself rather than round scores.
...
│   │   └── lambda\_function.py   — Google Sheets → weather → Parquet → S3

│   └── query/

│       └── lambda\_function.py   — DuckDB query handler for dashboard panels
...
│   └── server.py                — MCP server, execute\_sql + get\_course\_info tools
...
├── lambda\_function.py           — agent Lambda handler, secret key validation

├── deploy\_refresh.ps1           — deployment script for refresh Lambda (Windows)

├── deploy\_query.ps1             — deployment script for query Lambda (Windows)

├── deploy\_agent.ps1             — deployment script for agent Lambda (Windows)
...
AWS\_PROFILE=your-aws-profile

DATA\_BUCKET=your-s3-data-bucket-name
...
aws s3 cp s3://your-bucket/course\_meta.json data/course\_meta.json
...
python backend/query/lambda\_function.py
...
\- The AI chat interface requires a secret key configured via the `CHAT\_SECRET\_KEY` Lambda environment variable
